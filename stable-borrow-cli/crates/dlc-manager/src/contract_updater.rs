...
use bitcoin::{Amount, EcdsaSighashType, Sequence, TxIn, Txid};
    ...
    SP::Target: ContractSignerProvider<Signer = X>,
{
    ...
    let (mut collateral_tx, collateral_script_pubkey) = create_collateral_transaction(&borrower_pubkey, &lender_pubkey, total_collateral, escrow_input, offered_loan_contract.lender_hash).unwrap();
    ...
    //     offered_contract.offered_contract.fee_rate_per_vb,
    //     0,
    //     offered_loan_contract.offered_contract.cet_locktime,
    //     offered_loan_contract.offered_contract.fund_output_serial_id,
    // )?;
    ...
    let accept_msg: AcceptLoanDlc = AcceptLoanDlc {
        ...
        signed_escrow_spend_tx: collateral_tx,
    ...
    // let accept_msg: AcceptLoanDlc = accepted_contract.get_accept_contract_msg(&adaptor_sigs);
