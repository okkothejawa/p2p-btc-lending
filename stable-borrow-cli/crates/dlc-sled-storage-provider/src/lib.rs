...
use dlc_manager::contract::offered_contract::{OfferedContract, OfferedLoanContract};
...
const PREIMAGE_TREE: u8 = 9;
const ESCROW_TXID_TREE: u8 = 10;
...
impl SledStorageProvider {
    ...
    fn preimage_tree(&self) -> Result<Tree, Error> {
        self.open_tree(&[PREIMAGE_TREE])
    }

    fn escrow_txid_tree(&self) -> Result<Tree, Error> {
        self.open_tree(&[ESCROW_TXID_TREE])
    }
...
impl Storage for SledStorageProvider {
    ...
    fn create_loan_contract(&self, contract: &OfferedLoanContract) -> Result<(), Error> {
        let serialized = serialize_contract(&Contract::OfferedLoan(contract.clone()))?;
        self.contract_tree()?
            .insert(contract.offered_contract.id, serialized)
            .map_err(to_storage_error)?;
        Ok(())
    }

    fn set_loan_preimage(&self, contract_id: &ContractId, preimage: u128) -> Result<(), Error> {
        self.preimage_tree()?
            .insert(contract_id, &preimage.to_be_bytes())
            .map_err(to_storage_error)?;
        Ok(())
    }

    fn get_loan_preimage(&self, contract_id: &ContractId) -> Result<Option<u128>, Error> {
        match self
            .preimage_tree()?
            .get(contract_id)
            .map_err(to_storage_error)?
        {
            Some(res) => {
                // Convert IVec to a slice, then try to convert the slice to a fixed array
                let bytes: [u8; 16] = res.as_ref().try_into().map_err(|_| {
                    Error::StorageError("Invalid preimage data length, expected 16 bytes".to_string())
                })?;
    ...
    fn set_escrow_txid(&self, contract_id: &ContractId, txid: lightning::bitcoin::Txid) -> Result<(), Error> {
        self.escrow_txid_tree()?
            .insert(contract_id, &txid.to_byte_array())
            .map_err(to_storage_error)?;
        Ok(())
    }

    fn get_escrow_txid(&self, contract_id: &ContractId) -> Result<Option<lightning::bitcoin::Txid>, Error> {
        match self
            .escrow_txid_tree()?
            .get(contract_id)
            .map_err(to_storage_error)?
        {   
            Some(res) => Ok(Some(lightning::bitcoin::Txid::from_slice(&res).unwrap())),
            None => Ok(None),
        }
    }
    ...
    fn get_loan_contract_offers(&self) -> Result<Vec<OfferedLoanContract>, Error> {
        self.get_data_with_prefix(
            &self.contract_tree()?,
            &[ContractPrefix::OfferedLoan.into()],
            None,
        )
    }
...
fn serialize_contract(contract: &Contract) -> Result<Vec<u8>, lightning::io::Error> {
    let serialized = match contract {
        ...
        Contract::OfferedLoan(o) | Contract::OfferedLoanEscrowConfirmed(o) => o.serialize(),
...
fn deserialize_contract(buff: &sled::IVec) -> Result<Contract, Error> {
    ...
    let contract = match contract_prefix {
        ...
        ContractPrefix::OfferedLoan => {
            Contract::OfferedLoan(OfferedLoanContract::deserialize(&mut cursor).map_err(to_storage_error)?)
        }
        ContractPrefix::OfferedLoanEscrowConfirmed => {
            Contract::OfferedLoanEscrowConfirmed(
                OfferedLoanContract::deserialize(&mut cursor).map_err(to_storage_error)?,
            )
        }
