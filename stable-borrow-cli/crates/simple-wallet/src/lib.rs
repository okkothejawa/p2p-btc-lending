    ...
    W::Target: WalletStorage,
{
    ...
    fn sign_raw_transaction(
        ...
        // Collect all the inputs and signing data first
        let mut signing_data = Vec::new();
        
        // Get all UTXOs first to avoid temporary value drop
        ...
        for (i, input) in tx.input.iter().enumerate() {
            ...
            let utxo = utxos
                .iter()
                .find(|x| x.outpoint == *outpoint)
                .ok_or(Error::InvalidParameters(
                    "UTXO not found in wallet".to_string(),
                ))?;
            let address = Address::from_script(&utxo.tx_out.script_pubkey, self.network)
            ...
            signing_data.push((i, seckey, utxo.tx_out.value));
        }
        
        // Now sign each input one by one
        for (i, seckey, value) in signing_data {
