...
impl_type!(OFFER_LOAN_TYPE, OfferLoanDlc, 42779);
impl_type!(ACCEPT_TYPE, AcceptDlc, 42780);
impl_type!(ACCEPT_LOAN_TYPE, AcceptLoanDlc, 42781);
impl_type!(INITIATE_ACCEPT_LOAN_TYPE, InitiateAcceptLoanDlc, 42783);
...
/// Contains information about a OfferDlc and loan details
pub struct OfferLoanDlc {
    ...
    pub duration: u64,
    /// Lender's hashvalue to be used in collateral transaction.
    pub lender_hash: [u8; 32],
    /// OfferDlc message.
    pub offer_dlc: OfferDlc,
}

impl OfferLoanDlc {
    /// Returns the total collateral locked in the contract.
    pub fn get_total_collateral(&self) -> Amount {
        self.offer_dlc.get_total_collateral()
    }
    ...
    pub fn validate<C: Verification>(
        ...
        self.offer_dlc.validate(secp, min_timeout_interval, max_timeout_interval)
    }
}

impl_dlc_writeable!(OfferLoanDlc, {
    (collateral_ratio, writeable),
    (liquidation_ratio, writeable),
    (interest_rate, writeable),
    (duration, writeable),
    (lender_hash, writeable),
    (offer_dlc, writeable)
});
...
/// Contains information about a party wishing to accept a loan offer.
pub struct AcceptLoanDlc {
    /// The version of the protocol used by the peer.
    pub protocol_version: u32,
    #[cfg_attr(
        feature = "use-serde",
...
/// Contains information about after the stage of initial loan offer acceptance.
#[derive(Clone, Debug, PartialEq, Eq)]
#[cfg_attr(
    feature = "use-serde",
    derive(serde::Serialize, serde::Deserialize),
...
pub struct InitiateAcceptLoanDlc {
    /// The version of the protocol used by the peer.
    pub protocol_version: u32,
    #[cfg_attr(
        feature = "use-serde",
    ...
    /// The id of the contract referred to by this message.
    pub contract_id: [u8; 32],
    /// The borrower hash value.
    pub borrower_hash: [u8; 32],
    /// Txid of the escrow transaction
    pub escrow_txid: Txid,
}

impl_dlc_writeable!(InitiateAcceptLoanDlc, {
    (protocol_version, writeable),
    (contract_id, writeable),
    (borrower_hash, writeable),
    (escrow_txid, writeable)
...
pub enum Message {
    Offer(OfferDlc),
    OfferLoan(OfferLoanDlc),
    Accept(AcceptDlc),
    AcceptLoan(AcceptLoanDlc),
    InitiateAcceptLoan(InitiateAcceptLoanDlc),
    Sign(SignDlc),
